    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->myQGraphicsView->setScene(scene);                           //设置scene 为 scene
    ui->myQGraphicsView->setDragMode(QGraphicsView::ScrollHandDrag); //设置拖拽模
...
void MainWindow::drawTransLineshortstation(const QList<std::pair<QString,QString>>& trans_line)
{
    //绘图重置
    this->scene->clear();
    ui->myQGraphicsView->refresh();
    ...
    QString line_old = trans_line.at(0).second;
    QColor color = this->subsys.lines[line_old].color;
    drawEdge(this->subsys.stations[trans_line.at(0).first],this->subsys.stations[trans_line.at(1).first],color);
    drawStation(this->subsys.stations[trans_line.at(0).first],color);
    ui->textBrowser->setText("共经过"+QString::number(trans_line.size())+"站");
    ui->textBrowser->append("起点: "+trans_line.at(0).first);
    ui->textBrowser->append("        ↓ 乘坐"+this->subsys.lines[line_old].name);
    for(int i=1;i<trans_line.size()-1;i++){

        ui->textBrowser->append("      "+trans_line.at(i).first);
        if(trans_line.at(i).second != trans_line.at(i+1).second ){
              ui->textBrowser->append("        ↓ 换乘"+trans_line.at(i+1).second);
        }else{
              ui->textBrowser->append("        ↓");
        }

        drawEdge(this->subsys.stations[trans_line.at(i).first],this->subsys.stations[trans_line.at(i+1).first],this->subsys.lines[trans_line.at(i+1).second].color);
        drawStation(this->subsys.stations[trans_line.at(i).first],color);
        color = this->subsys.lines[trans_line.at(i+1).second].color;

    }
    drawStation(this->subsys.stations[trans_line.last().first],this->subsys.lines[trans_line.last().second].color);
    ui->textBrowser->append("终点: "+trans_line.last().first);
...
void MainWindow::queryLine()
{
    ...
    QList<std::pair<QString,QString> >ll;
    if(ui->radioButton->isChecked()){
        ...
        ll = this->subsys.lessTransPath(start_sta,end_sta);
        drawTransLineshortstation(ll);
    }
